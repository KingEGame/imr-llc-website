...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    resolve: {
      ...
      // Ensure a single React instance across the module graph.
      dedupe: ['react', 'react-dom'],
    },
    // Pre-bundle three at startup so adding it doesn't trigger a mid-session
    // dep re-optimization (which left React's dispatcher in a bad state).
    optimizeDeps: {
